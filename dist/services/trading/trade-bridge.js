...
class TradeBridgeService {
    constructor(port = 8080) {
        ...
        this.memoryQueue = new Map(); // Fallback for Redis
        // Check if Redis should be used
        this.useMemoryCache = process.env.USE_MEMORY_CACHE === 'true' || !process.env.REDIS_URL || process.env.REDIS_URL === 'optional';
        // Initialize Redis only if available and needed
        if (!this.useMemoryCache && process.env.REDIS_URL && process.env.REDIS_URL !== 'optional') {
            try {
    ...
    async handleTradeOpened(connectionId, signal) {
        ...
        try {
            ...
            await this.publishUpdate('trade_updates', {
                type: 'trade_opened',
    ...
    async sendCopyInstruction(instruction) {
        ...
        if (targetConnection) {
            ...
            // Queue instruction for when account comes online
            await this.queueInstruction(instruction.targetAccountId, instruction);
    ...
    setupRedisSubscriptions() {
        if (!this.redis) {
            console.log('📝 Redis subscriptions disabled (using memory cache)');
            return;
        }
        try {
            ...
            console.warn('⚠️ Redis subscription setup failed:', error);
        }
    }
    ...
    // Helper methods for optional Redis
    async publishUpdate(channel, data) {
        ...
        // Could add WebSocket broadcast to dashboard here
    }
    async queueInstruction(accountId, instruction) {
        if (this.redis) {
            try {
                await this.redis.lpush(`copy_queue:${accountId}`, JSON.stringify(instruction));
            }
            catch (error) {
                console.warn('Redis queue failed:', error);
                this.queueInstructionMemory(accountId, instruction);
            }
        }
        else {
            this.queueInstructionMemory(accountId, instruction);
    ...
    queueInstructionMemory(accountId, instruction) {
        const queueKey = `copy_queue:${accountId}`;
        if (!this.memoryQueue.has(queueKey)) {
            this.memoryQueue.set(queueKey, []);
        }
        this.memoryQueue.get(queueKey).push(instruction);
        // Keep only last 100 instructions per account
...
exports.tradeBridge = new TradeBridgeService();
